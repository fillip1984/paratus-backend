...
import { handlers, isSecureContext } from "~/server/auth/index";
...
export const GET = async (
  ...
  console.log({ msg: "inside auth api", _req });

  // First step must be to correct the request URL.
  const req = rewriteRequestUrlInDevelopment(_req);
  console.log({ msg: "inside auth api, rewritten", req });
  ...
  if (nextauthAction === "signin" && !!isExpoSignIn) {
    console.log({ msg: "inside auth api, cookie being set" });
